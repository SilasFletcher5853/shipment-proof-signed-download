...
The decision has to come first: you only mint a five-minute signed download URL when the latest shipment event is`delivered`, every exception is resolved, and the proof object actually exists. Infrai gives you that presigned storage call behind a single`INFRAI_API_KEY`, and the same key covers every capability an agent workflow adds later, so the tool can ask for a narrowly scoped link without ever holding a storage credential.
...
The URL is the concrete tool result: a client does a`GET`against it before expiry, while the service keeps the API key and object location policy server-side.
